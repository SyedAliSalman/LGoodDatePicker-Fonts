...
/**
 * This class represents an oval button. This means that the button will only trigger an acton if it
 * is clicked within the oval shape drawn on it. The button can be also of circular shape (if
 * width=height), or capsule shape (either horizontal or vertical capsule).
 *
 * @author Luka Kralj
...
public class OvalButton extends JButton implements MouseListener, MouseMotionListener {
   ...
   * @param colorBorderHighlighted The border color that will show whenever the button is marked as
   *     being highlighted (will remain this color even after the mouse exits).
   */
  public OvalButton(
      int shape,
      int orientation,
      ...
      Color colorBorderNormal,
      Color colorBorderHighlighted) {
    super();
    ...
    if (orientation != VERTICAL && orientation != HORIZONTAL) {
      throw new IllegalArgumentException(
          "Invalid orientation chosen for OvalButton: " + orientation);
    }
    this.shape = shape;
  ...
  /**
   * Set the border color that will show whenever the button is marked as being highlighted (will
   * remain this color even after the mouse exits). Independent of hover highlighting.
   *
  ...
  /**
   * Set radius of the capsule-shape button. Setting radius to 1 produces the same effect as
   * SHAPE_OVAL. Setting radius to 0 makes the button rectangular. Setting radius of the oval-shaped
  ...
  /**
   * This method is overridden because it adds additional checks before executing the action
   * specified by the user of the button.
   *
   ...
   *
   * @param screenPosition Position we want to check. It must be a position on the screen - not
   *     component dependent.
  ...
  /**
   * Should return the image that we want to set as the background. This image will be rendered
   * within the oval/capsule shape of the button, but not outside of it. Return null if you do not
   * want image as the background. In this case a background color will be used.
   *
   ...
   * @return True if the point is within the borders or on the border of the oval, false if it is
   *     outside of it.
   */
   ...
   *
   * @param p Point to check.
   * @return True if the point is within the borders or on the border of the capsule, false if it is
   *     outside of it.
   */
...
class CircularJLabel extends JLabel {
  public CircularJLabel(String text) {
    super(text);
  }
  ...
  protected void paintComponent(Graphics g) {
    ...
    // Create a circular shape using Ellipse2D
    Shape circle = new Ellipse2D.Double(0, 0, getWidth(), getHeight());

    // Get the graphics context and set the color and stroke for the circle
    Graphics2D g2d = (Graphics2D) g;
    g2d.setColor(Color.BLACK); // Change the color as desired
    g2d.setStroke(new BasicStroke(2)); // Adjust the stroke width as needed
...
class CircularBackgroundLabel extends JPanel {
  private Color backgroundColor;

  public CircularBackgroundLabel(Color backgroundColor) {
